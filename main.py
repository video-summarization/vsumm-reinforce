...
from logger_tf import Logger as Logger_tb
...
# Dataset options
parser.add_argument( '--dataset', default="datasets/eccv16_dataset_summe_google_pool5.h5" , type=str, help="path to h5 dataset (required)")
parser.add_argument( '--split', default="datasets/summe_splits.json" ,type=str, help="path to split file (required)")
parser.add_argument('--split-id', type=int, default=0, help="split index (default: 0)")
parser.add_argument( '--metric', default="summe", type=str, choices=['tvsum', 'summe'],help="evaluation metric ['tvsum', 'summe']")
# Model options
...
parser.add_argument('--save-dir', default="log/summe-split0" ,type=str, help="path to save output (default: 'log/')")
...
#1. train on 1 video at a time (back prop step)
#2. evaluation happens without sampling : on scores
...
def main():
    ...
    #logger
    logger_tf = Logger_tb('./logs/split_'+str(args.split_id))
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for idx in idxs:
            ...
            for _ in range(args.num_episode): #same video multiple episodes
                actions = m.sample()
                log_probs = m.log_prob(actions)
                reward, reward_div, reward_rep = compute_reward(seq, actions, use_gpu=use_gpu)
            ...
            ####################################### plot summaries #######################################
            logger_tf.scalar_summary('cost', cost.data.cpu().numpy().item(), counter)
            logger_tf.scalar_summary('total_reward', reward.data.cpu().numpy().item(), counter)
            logger_tf.scalar_summary('reward_div', reward_div.data.cpu().numpy().item(), counter)
            logger_tf.scalar_summary('reward_rep', reward_rep.data.cpu().numpy().item(), counter)
            counter = counter + 1

            # 2. Log values and gradients of the parameters (histogram summary)
            for tag, value in model.named_parameters():
                tag = tag.replace('.', '/')
                logger_tf.histo_summary(tag, value.data.cpu().numpy(), counter)
                logger_tf.histo_summary(tag + '/grad', value.grad.data.cpu().numpy(), counter)
            #############################################################################################
